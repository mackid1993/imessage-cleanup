...
impl IDSUser {
    ...
    /// Returns the raw bytes from the id-get-dependent-registrations endpoint.
    pub async fn get_dependent_registrations_raw(&self, aps: &APSState) -> Result<Vec<u8>, PushError> {
    ...
    /// Remove a single device's registration by its push token.
    ///
    /// Tries multiple signing/header combinations. Each approach logs its response.
    /// Does NOT short-circuit on status 0 from approaches that might be false positives
    /// (operating on our own device instead of the target).
    pub async fn deregister_device(&self, aps: &APSState, config: &dyn OSConfig, target_token: &[u8]) -> Result<u64, PushError> {
        let target_b64 = base64_encode(target_token);
        let our_b64 = base64_encode(aps.token.as_ref().unwrap());
        info!("=== Deregistering device target={}... our={}... ===",
            &target_b64[..16.min(target_b64.len())],
            &our_b64[..16.min(our_b64.len())]);

        async fn try_request(label: &str, request: SignedRequest<Signed>) -> Result<(u64, String), PushError> {
            let resp = request.send(&REQWEST).await?;
            let http_status = resp.status().as_u16();
            let body_bytes = resp.bytes().await?;
            let body_str = String::from_utf8_lossy(&body_bytes).to_string();
            let inner_status = plist::from_bytes::<Value>(&body_bytes).ok()
                .and_then(|v| v.as_dictionary()?.get("status")?.as_unsigned_integer());
            let s = inner_status.unwrap_or(9999);
            info!("[{}] HTTP {} | IDS status {} | {}", label, http_status, s, &body_str[..body_str.len().min(500)]);
            Ok((s, body_str))
        }

        let register_meta = config.get_register_meta();
        let mut best_status = 9999u64;

        // === Approach E: id-deregister, valid push+auth sigs (our certs, our token), TARGET in x-push-token ===
        // Push sig authenticates us as a real device. Auth sig proves account.
        // x-push-token tells the server which device to operate on.
        // Key test: does the server use x-push-token as target even if push sig token differs?
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
                ("validation-data".to_string(), Value::Data(vd)),
            ])))?)?;
            let (s, _) = try_request("E: id-deregister push+auth(ours), x-push-token=TARGET",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &target_b64)
            ...
            if s == 0 { info!("*** Approach E returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        // === Approach F: id-register empty services, push+auth sigs (our certs, our token), TARGET in x-push-token ===
        // Apple's register-as-deregister but redirected to a different device via x-push-token.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("device-name".to_string(), Value::String(config.get_device_name())),
                ("hardware-version".to_string(), Value::String(register_meta.hardware_version.clone())),
                ("language".to_string(), Value::String("en-US".to_string())),
                ("os-version".to_string(), Value::String(register_meta.os_version.clone())),
                ("private-device-data".to_string(), Value::Dictionary(config.get_private_data())),
                ("services".to_string(), Value::Array(vec![])),
                ("software-version".to_string(), Value::String(register_meta.software_version.clone())),
                ("validation-data".to_string(), Value::Data(vd)),
            ])))?)?;
            let (s, _) = try_request("F: id-register empty svc, push+auth(ours), x-push-token=TARGET",
                SignedRequest::new("id-register", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id-0", &self.user_id)
                    .header("user-agent", &format!("com.apple.invitation-registration {}", config.get_version_ua()))
                    .header("content-type", "application/x-apple-plist")
                    .header("content-encoding", "gzip")
                    .body(body)
            ...
            if s == 0 { info!("*** Approach F returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        // === Approach G: id-deregister, push signs TARGET token (our keypair), auth signs TARGET ===
        // Both sigs use target token in payload. Push cert won't match target token,
        // but server may only validate RSA signature, not cert↔token correspondence.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            let (s, _) = try_request("G: id-deregister push(target)+auth(target), both our keypairs",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
                    ...
                    .header("content-encoding", "gzip")
                    .body(body)
                    .sign_with_token(&self.auth_keypair, KeyType::Auth, target_token, None)?
                    .sign_with_token(aps.keypair.as_ref().unwrap(), KeyType::Push, target_token, None)?
            ).await?;
            if s == 0 { info!("*** Approach G returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        // === Approach H: id-register empty services, push(target)+auth(target), our keypairs ===
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("device-name".to_string(), Value::String(config.get_device_name())),
            ...
            let (s, _) = try_request("H: id-register empty svc, push(target)+auth(target), our keypairs",
                SignedRequest::new("id-register", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id-0", &self.user_id)
                    ...
                    .header("content-encoding", "gzip")
                    .body(body)
                    .sign_with_token(&self.auth_keypair, KeyType::Auth, target_token, Some(0))?
                    .sign_with_token(aps.keypair.as_ref().unwrap(), KeyType::Push, target_token, None)?
            ).await?;
            if s == 0 { info!("*** Approach H returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        // === Approach I: id-deregister, push signs OUR, auth signs TARGET, x-push-token=TARGET ===
        // Mixed: push authenticates us, auth targets the device
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            let (s, _) = try_request("I: id-deregister push(ours)+auth(target), x-push-token=TARGET",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
            ...
            if s == 0 { info!("*** Approach I returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        // === Approach J: id-register empty services, push(ours)+auth(target), x-push-token=TARGET ===
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("device-name".to_string(), Value::String(config.get_device_name())),
            ...
            let (s, _) = try_request("J: id-register empty svc, push(ours)+auth(target), x-push-token=TARGET",
                SignedRequest::new("id-register", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id-0", &self.user_id)
            ...
            if s == 0 { info!("*** Approach J returned status 0! ***"); return Ok(0); }
            best_status = best_status.min(s);
        }

        info!("=== All approaches failed. Best status: {} ===", best_status);
        Ok(best_status)
    }

    /// Deregister ourselves by sending id-register with empty services.
    /// Approach N: id-deregister with OUR push token in header (so push cert validates),
    /// but TARGET token in the body. Requires id-cert from registration.
    /// The id-cert proves we're a registered device, granting permission to target
    /// a different device via the body's push-token field.
    pub async fn deregister_target_n(&self, aps: &APSState, config: &dyn OSConfig, target_token: &[u8]) -> Result<u64, PushError> {
        let target_b64 = base64_encode(target_token);
        info!("=== Approach N: id-deregister, our token in header, target in body, token={}... ===",
            &target_b64[..16.min(target_b64.len())]);

        let id_keypair = self.registration.get("com.apple.madrid")
            .ok_or_else(|| PushError::RegisterFailed(0, "Not registered for com.apple.madrid".into()))?
            .id_keypair.clone();
        ...
        let request = SignedRequest::new("id-deregister", Method::POST)
            .header("x-push-token", &base64_encode(aps.token.as_ref().unwrap()))
            .header("x-protocol-version", &config.get_protocol_version().to_string())
            .header("x-auth-user-id", &self.user_id)
            .header("user-agent", &format!("com.apple.invitation-registration {}", config.get_version_ua()))
            ...
            .header("content-encoding", "gzip")
            .body(body)
            .sign(&id_keypair, KeyType::Id, aps, None)?
            .sign(&self.auth_keypair, KeyType::Auth, aps, None)?
            .sign(aps.keypair.as_ref().unwrap(), KeyType::Push, aps, None)?;
        ...
        let inner_status = plist::from_bytes::<Value>(&body_bytes).ok()
            .and_then(|v| v.as_dictionary()?.get("status")?.as_unsigned_integer())
            .unwrap_or(9999);
        info!("[N] HTTP {} | IDS status {} | {}", http_status, inner_status, &body_str[..body_str.len().min(500)]);
        Ok(inner_status)
    }

    /// This is the standard Apple _sendRegistrationAsDeregister: approach.
    /// Only works for OUR device (matching push cert).
    pub async fn deregister_self(&self, aps: &APSState, config: &dyn OSConfig) -> Result<(), PushError> {
        info!("=== Deregistering self (register with empty services) ===");
        let register_meta = config.get_register_meta();
        let vd = config.generate_validation_data().await?;
        let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
            ("device-name".to_string(), Value::String(config.get_device_name())),
            ("hardware-version".to_string(), Value::String(register_meta.hardware_version)),
            ("language".to_string(), Value::String("en-US".to_string())),
            ("os-version".to_string(), Value::String(register_meta.os_version)),
            ("private-device-data".to_string(), Value::Dictionary(config.get_private_data())),
            ("services".to_string(), Value::Array(vec![])),
            ("software-version".to_string(), Value::String(register_meta.software_version)),
            ("validation-data".to_string(), Value::Data(vd)),
        ])))?)?;

        let request = SignedRequest::new("id-register", Method::POST)
            .header("x-push-token", &base64_encode(aps.token.as_ref().unwrap()))
            .header("x-protocol-version", &config.get_protocol_version().to_string())
            .header("x-auth-user-id-0", &self.user_id)
            .header("user-agent", &format!("com.apple.invitation-registration {}", config.get_version_ua()))
            ...
            .body(body)
            .sign(aps.keypair.as_ref().unwrap(), KeyType::Push, aps, None)?
            .sign(&self.auth_keypair, KeyType::Auth, aps, Some(0))?;

        let resp = request.send(&REQWEST).await?;
        let status = resp.status().as_u16();
        let body_bytes = resp.bytes().await?;
        let body_str = String::from_utf8_lossy(&body_bytes).to_string();
        info!("deregister_self: HTTP {} | {}", status, &body_str[..body_str.len().min(300)]);
        Ok(())
    }

    /// Try deregistering another device using id-cert (registration identity cert).
    /// Requires this user to be registered first (call register() beforehand).
    pub async fn deregister_with_id_cert(&self, aps: &APSState, config: &dyn OSConfig, target_token: &[u8]) -> Result<u64, PushError> {
        let target_b64 = base64_encode(target_token);
        info!("=== Deregistering target with id-cert, token={}... ===", &target_b64[..16.min(target_b64.len())]);
        ...
        // === K: id-deregister, id+auth signing (no push), target in x-push-token ===
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            let (s, _) = try_request("K: id-deregister id+auth, x-push-token=TARGET",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
                    ...
                    .header("content-encoding", "gzip")
                    .body(body)
                    .sign_with_token(&id_keypair, KeyType::Id, target_token, None)?
                    .sign_with_token(&self.auth_keypair, KeyType::Auth, target_token, None)?
            ).await?;
            if s == 0 { return Ok(0); }
        }

        // === L: id-deregister, id+auth+push signing, target in x-push-token ===
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            let (s, _) = try_request("L: id-deregister id+auth+push, x-push-token=TARGET",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
        ...
        // === M: id-register empty services, id+auth+push, target in x-push-token ===
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("device-name".to_string(), Value::String(config.get_device_name())),
            ...
            let (s, _) = try_request("M: id-register empty svc, id+auth+push, x-push-token=TARGET",
                SignedRequest::new("id-register", Method::POST)
                    .header("x-push-token", &target_b64)
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id-0", &self.user_id)
        ...
        // === N: id-deregister, id+push (our token) + auth, x-push-token=OUR, target in body ===
        // Use our push token in header (so push cert matches), but target in body.
        // Id cert proves we're registered. Maybe with id-cert the body's push-token is respected.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            let (s, _) = try_request("N: id-deregister id+push+auth(ours), target in body only",
                SignedRequest::new("id-deregister", Method::POST)
                    .header("x-push-token", &base64_encode(aps.token.as_ref().unwrap()))
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
        ...
        // === O: id-deregister GET (like get-dependent-registrations but deregister) ===
        // Signed with id+push+auth, all our tokens. Maybe GET deregisters ALL.
        {
            let (s, _) = try_request("O: id-deregister GET, id+push+auth(ours)",
                SignedRequest::new("id-deregister", Method::GET)
                    .header("x-push-token", &base64_encode(aps.token.as_ref().unwrap()))
                    .header("x-protocol-version", &config.get_protocol_version().to_string())
                    .header("x-auth-user-id", &self.user_id)
                    .sign(&id_keypair, KeyType::Id, aps, None)?
        ...
        info!("=== All id-cert approaches failed ===");
        Ok(6001)
    }

    /// Try deregistering another device via APNs tunnel.
    /// The APNs connection authenticates us at the transport level, so the server
    /// might not re-validate push cert ↔ x-push-token in the tunneled HTTP request.
    pub async fn deregister_via_apns(&self, conn: &APSConnectionResource, config: &dyn OSConfig, target_token: &[u8]) -> Result<u64, PushError> {
        let target_b64 = base64_encode(target_token);
        info!("=== Trying APNs tunnel approaches for target={}... ===", &target_b64[..16.min(target_b64.len())]);

        let aps = conn.state.read().await;
        let register_meta = config.get_register_meta();
        ...
        // === P: id-register empty services via APNs, id+auth+push, x-push-token=TARGET ===
        // Through APNs tunnel, the push connection authenticates us.
        // The server may accept a different x-push-token since transport auth is separate.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("device-name".to_string(), Value::String(config.get_device_name())),
            ...
            match request.send_apns(conn, "com.apple.madrid").await {
                Ok(resp_bytes) => {
                    let body_str = String::from_utf8_lossy(&resp_bytes).to_string();
                    let inner_status = plist::from_bytes::<Value>(&resp_bytes).ok()
                        .and_then(|v| v.as_dictionary()?.get("status")?.as_unsigned_integer());
                    let s = inner_status.unwrap_or(9999);
                    info!("[P: APNs id-register empty svc, x-push-token=TARGET] IDS status {} | {}", s, &body_str[..body_str.len().min(500)]);
                    if s == 0 { return Ok(0); }
                }
                Err(e) => {
                    info!("[P: APNs id-register empty svc] Error: {}", e);
                }
            }
        }

        // === Q: id-deregister via APNs, auth+push (our token), target in body ===
        // Maybe through APNs, the body's push-token IS respected for targeting.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            match request.send_apns(conn, "com.apple.madrid").await {
                Ok(resp_bytes) => {
                    ...
                    let inner_status = plist::from_bytes::<Value>(&resp_bytes).ok()
                        .and_then(|v| v.as_dictionary()?.get("status")?.as_unsigned_integer());
                    let s = inner_status.unwrap_or(9999);
                    info!("[Q: APNs id-deregister, our token, target in body] IDS status {} | {}", s, &body_str[..body_str.len().min(500)]);
                    if s == 0 { return Ok(0); }
                }
                Err(e) => {
                    info!("[Q: APNs id-deregister] Error: {}", e);
                }
            }
        }

        // === R: id-deregister via APNs, auth-only (no push sig), x-push-token=TARGET ===
        // Through APNs, push auth is implicit. Maybe push sig isn't needed at all.
        {
            let vd = config.generate_validation_data().await?;
            let body = gzip_normal(&plist_to_buf(&Value::Dictionary(Dictionary::from_iter([
                ("push-token".to_string(), Value::Data(target_token.to_vec())),
            ...
            match request.send_apns(conn, "com.apple.madrid").await {
                Ok(resp_bytes) => {
                    ...
                    let inner_status = plist::from_bytes::<Value>(&resp_bytes).ok()
                        .and_then(|v| v.as_dictionary()?.get("status")?.as_unsigned_integer());
                    let s = inner_status.unwrap_or(9999);
                    info!("[R: APNs id-deregister auth-only, x-push-token=TARGET] IDS status {} | {}", s, &body_str[..body_str.len().min(500)]);
                    if s == 0 { return Ok(0); }
                }
                Err(e) => {
                    info!("[R: APNs id-deregister auth-only] Error: {}", e);
                }
            }
        }

        info!("=== All APNs tunnel approaches failed ===");
        Ok(6001)
    }

    /// Deregister multiple devices one by one.
    pub async fn deregister_all_devices(&self, aps: &APSState, config: &dyn OSConfig, target_tokens: &[Vec<u8>]) -> Result<(), PushError> {
        info!("=== Deregistering {} devices one by one ===", target_tokens.len());
        let mut success_count = 0u32;
        for (i, token) in target_tokens.iter().enumerate() {
            info!("--- Device {}/{} ---", i + 1, target_tokens.len());
            match self.deregister_device(aps, config, token).await {
                Ok(0) => {
                    success_count += 1;
                    info!("Device {}/{}: status 0", i + 1, target_tokens.len());
                }
                Ok(s) => {
                    info!("Device {}/{}: status {}", i + 1, target_tokens.len(), s);
                }
                Err(e) => {
                    error!("Device {}/{}: error: {}", i + 1, target_tokens.len(), e);
                }
            }
        }
        info!("=== Batch complete. {}/{} returned status 0. ===", success_count, target_tokens.len());
        Ok(())
    }
