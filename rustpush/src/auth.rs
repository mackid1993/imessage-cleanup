...
impl<S: RequestState> SignedRequest<S> {
    ...
    pub fn sign(mut self, pair: &KeyPairNew<RsaKey>, key_type: KeyType, aps: &APSState, item: Option<usize>) -> Result<SignedRequest<Signed>, PushError> {
        self.sign_with_token(pair, key_type, aps.token.as_ref().unwrap(), item)
    }

    /// Sign with an explicit token in the payload instead of using aps.token.
    /// Used for deregistration where the target device's token must be in the payload.
    pub fn sign_with_token(mut self, pair: &KeyPairNew<RsaKey>, key_type: KeyType, token: &[u8], item: Option<usize>) -> Result<SignedRequest<Signed>, PushError> {
